$(document).ready(function(){
	...
	var userscompany = $('#company');
			...
			//CHECKING COMPANY WAS SELECTED
			if (userssubject.val() == "") {
				
				$(".error").html('Please, provide your company name.');
				$(".error").delay(1200).fadeIn(1000).delay(4000).fadeOut(1000);
				$('input[type=submit]', $("#lcars-contact-beta")).removeAttr('disabled');
				return false;
			}
			...
			*/
			...
			$.post(url,{ usersname: usersname.val(), usersemail: usersemail.val(), usersphone: usersphone.val(), userscompany: userscompany.val(), userssubject: userssubject.val(),userscomment: userscomment.val(), usershenanigans: usershenanigans.val(), isvalid: isvalid } , function(data) {
